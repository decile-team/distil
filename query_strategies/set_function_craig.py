...
class PerClassNonDeepSetFunction(object):
    ...
    def lazy_greedy_max(self, budget, model_params):
        ...
        # print('DEBUG')
        # print('-------------------------')
        # print('Total Dataset Points - ', self.x_trn.shape[0])
        # print('Overall Budget - ', budget)
        # print('Individual Class Budget - ', per_class_bud)
        # print('Unique points in target -', classes)
        # print('Count of individual datapoints as per class above - ', no_elements)

        for i in classes:
            idxs = torch.where(self.y_trn == i)[0]
            no_per_class_bud = min(per_class_bud, len(idxs)) #If data points per class are less than per_class_bud

            self.compute_score(model_params, idxs)
            fl = apricot.functions.facilityLocation.FacilityLocationSelection(random_state=0, metric='precomputed',
                                                                              n_samples=no_per_class_bud)
            ...
            total_greedy_list.extend(idxs[greedyList])
            gammas.extend(gamma)
            # print('Length of greedy list ', len(greedyList))
